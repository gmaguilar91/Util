...
class subirMuchos {
    ...
    private $destino = "./", $nombre = "", $tamano = "100000000", $parametro, $extension, $lista;
    ...
    function __construct($lista, $parametro) {
        if (isset($lista)) {
            $this->parametro = $parametro;
            $this->lista = $lista;

            $nombre = $lista[$this->parametro]["name"];
    ...
    public function upload() {
       if ($this->error) {
        ...
        if ($this->lista[$this->parametro]["error"] != UPLOAD_ERR_OK) {
            return false;
        }
        if ($this->lista[$this->parametro]["size"] > $this->tamano) {
            return false;
        }
        ...
        return move_uploaded_file($this->lista[$this->parametro]["tmp_name"], $this->destino . $nombre . "." . $this->extension);
    }
    ...
    public function isTipo($tipo) {
        return isset($this->arrayTipos[$tipo]);
    }
